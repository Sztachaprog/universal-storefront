...
def test_create_user(cursor):
    register_user("user", "password", "user@example.com", is_premium=True, cursor=cursor)
    cursor.execute("SELECT username, password_hash, email, is_premium FROM users WHERE username = 'user';")
    user = cursor.fetchone()
    
    assert user is not None, "User not found in database."
    assert user[0] == "user", f"Where looking for 'bartek', found '{user[0]}'"
...
def test_duplicate_username(cursor):
    register_user("user1", "password", "user1@example.com", cursor = cursor)

    with pytest.raises(ValueError):
        register_user("user1", "password", "user2@example.com", is_premium=True, cursor = cursor)

def test_duplicate_email(cursor):
    register_user("user1", "password", "user1@example.com", cursor = cursor)

    with pytest.raises(ValueError):
        register_user("user2", "password", "user1@example.com", is_premium=True, cursor = cursor)
# Read
